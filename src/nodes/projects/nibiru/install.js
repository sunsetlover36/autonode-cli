...
export const installScript = ({ customPorts }) => {
  ...
  const moniker = `nibiru-${generateRandomStr(12)}`;
  return {
    script: `cd || return
rm -rf ${gitFolder}
git clone https://github.com/NibiruChain/${gitFolder}.git
cd ${gitFolder} || return
...
curl -s ${genesisUrl} | jq -r .result.genesis > $HOME/.${projectFolder}/config/genesis.json
