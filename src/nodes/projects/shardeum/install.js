...
import { getAvailablePort, log } from "@utils";

export const installScript = async () => {
  const dashboardPort = await getAvailablePort(8080);
  const p2pFirstPort = await getAvailablePort(9001);
  const p2pSecondPort = await getAvailablePort(10001);

  const dashboardPassword = generateRandomStr(12);
  log(
    chalk.cyanBright(
      `Save your dashboard password for Shardeum: ${chalk.magenta.bold(
        dashboardPassword
      )}`
    )
  );
...
RUNDASHBOARD=y
  ...
  DASHPASS=\$(hash_password "${dashboardPassword}")

DASHPORT=${dashboardPort}
EXTERNALIP="auto"
INTERNALIP="auto"
SHMEXT=${p2pFirstPort}
SHMINT=${p2pSecondPort}
NODEHOME=~/.shardeum
