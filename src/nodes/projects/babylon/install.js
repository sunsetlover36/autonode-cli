...
export const installScript = ({ customPorts }) => {
  ...
  const moniker = `babylon-${generateRandomStr(12)}`;
  return {
    script: `cd || return
rm -rf ${gitFolder}
git clone https://github.com/babylonchain/${gitFolder}.git
cd ${gitFolder} || return
...
curl -L ${genesisUrl} > genesis.tar.bz2
...
sed -i 's|^network *=.*|network = "mainnet"|g' $HOME/.${projectFolder}/config/app.toml
sed -i 's|^checkpoint-tag *=.*|checkpoint-tag = "bbn0"|g' $HOME/.${projectFolder}/config/app.toml
